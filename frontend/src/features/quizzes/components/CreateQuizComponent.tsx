...
const levels = [
    ...
    { title: "medium", color: "accent-blue-500" },
    { title: "hard", color: "accent-orange-500" },
    { title: "expert", color: "accent-red-500" },
];

const questionsCountOptions: string[] = ["Auto", "10", "20", "30", "50", "70", "100"];
...
const CreateQuizComponent = () => {
    ...
    const handleOnSubmit = (e: FormEvent) => {
        ...
        if (questionsCountOptions.filter((item) => item === questionsCount).length === 0) {
    ...
    return (
        <form onSubmit={handleOnSubmit} className="grid w-[40vw] max-w-xl gap-4  text-black py-4">
                ...
                <div className="flex justify-between text-sm font-medium mt-2">
                    {levels.map((lvl, i) => {
                        return (
                            <span
                                key={i}
                                className={cn("capitalize", i === value ? `text-blue-500 font-bold` : "text-gray-500")}
                            >
                                {lvl.title}
                            </span>
                        );
            ...
            <div className="w-full flex flex-row justify-between ">
                ...
                <div className="flex items-center justify-between space-x-2 ">
                    <Button variant="outline" type="button" onClick={() => setShowIconPicker(true)}>
                        {selectedIcon ? (
                            <>
            ...
            <div className="w-full flex flex-col mb-2">
                <label className="font-semibold text-sm mb-2">Additional data (optional)</label>
                <Textarea
            ...
            {showIconPicker && <SelectIcon setSelectedIcon={setSelectedIcon} setShowIconPicker={setShowIconPicker} />}
            <div className="flex flex-row justify-end">
                <Button type="submit">{isPending ? "Uploading ..." : "Create quiz"}</Button>
            </div>
        </form>
    );
};
