...
const QuestionSection = ({
    ...
    const [favorite, setFavorite] = useState(questions[questionNumber].isFavorite || false);
    ...
    const handleSelectAnswer = (index: number) => {
        ...
        if (answer === -1) {
            ...
            status = questions[question].answers[answer].correct ? "correct" : "incorrect";
        }

        setAnswers((prev) => [...prev, { question: questions[question].question, status, isFavorite }]);
        ...
        <AnimatePresence mode="wait">
            <div className="w-full h-full overflow-scroll flex flex-col gap-5">
                <div className="flex flex-row justify-between items-center  font-semibold text-slate-500">
                    <p>
                        {t("page.questions")} {questionNumber + 1} {t("page.of")} {questions.length}
                    </p>
                    ...
                    <div className="w-fit flex flex-row gap-5 items-center">
                        <div>
                            <Timer end={showSummary} handleFinish={(item) => setTimeLeft(item)} />
                        </div>
                        {Math.floor((questionNumber * 100) / questions.length)}% {t("page.complete")}
                    </div>
                </div>
                ...
                >
                    ...
                    <p className="font-semibold text-xl">{questions[questionNumber].question}</p>
